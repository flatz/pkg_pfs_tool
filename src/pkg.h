...
enum content_type {
	...
	//CONTENT_TYPE_MD = 0x1D,
	CONTENT_TYPE_DP = 0x1E, /* pkg_ps4_delta_patch */
	//CONTENT_TYPE_MD = 0x1F,
};
...
TYPE_BEGIN(struct pkg_header, PKG_HEADER_SIZE);
	TYPE_FIELD(uint8_t magic[4], 0x00);
	TYPE_FIELD(uint32_t flags, 0x04);
#		define PKG_FLAGS_KEY_VER_1    (UINT32_C(1) << 24)
#		define PKG_FLAGS_SIGNED_EKPFS (UINT32_C(1) << 25)
#		define PKG_FLAGS_INTERNAL     (UINT32_C(1) << 30)
#		define PKG_FLAGS_FINALIZED    (UINT32_C(1) << 31)
	TYPE_FIELD(uint32_t unk_0x08, 0x08);
	TYPE_FIELD(uint32_t unk_0x0C, 0x0C); /* 0xF */
	TYPE_FIELD(uint32_t entry_count, 0x10);
	TYPE_FIELD(uint16_t sc_entry_count, 0x14);
	TYPE_FIELD(uint16_t entry_count_2, 0x16); /* same as entry_count */
	TYPE_FIELD(uint32_t entry_table_offset, 0x18);
	TYPE_FIELD(uint32_t main_ent_data_size, 0x1C);
	TYPE_FIELD(uint64_t body_offset, 0x20);
	TYPE_FIELD(uint64_t body_size, 0x28);
	TYPE_FIELD(char content_id[PKG_CONTENT_ID_SIZE], 0x40);
	TYPE_FIELD(uint32_t drm_type, 0x70);
...
#		define PKG_CONTENT_FLAGS_CUMULATIVE_PATCH 0x60000000
	TYPE_FIELD(uint32_t promote_size, 0x7C);
	TYPE_FIELD(uint32_t version_date, 0x80); /* pub tool version date */
	TYPE_FIELD(uint32_t version_hash, 0x84); /* pub tool version hash */
	TYPE_FIELD(uint32_t unk_0x88, 0x88); /* for delta patches only? */
	TYPE_FIELD(uint32_t unk_0x8C, 0x8C); /* for delta patches only? */
	TYPE_FIELD(uint32_t unk_0x90, 0x90); /* for delta patches only? */
	TYPE_FIELD(uint32_t unk_0x94, 0x94); /* for delta patches only? */
	TYPE_FIELD(uint32_t iro_tag, 0x98);
	TYPE_FIELD(uint32_t ekc_version, 0x9C); /* drm type version */
	TYPE_FIELD(uint8_t sc_entries1_hash[PKG_HASH_SIZE], 0x100);
	TYPE_FIELD(uint8_t sc_entries2_hash[PKG_HASH_SIZE], 0x120);
	TYPE_FIELD(uint8_t digest_table_hash[PKG_HASH_SIZE], 0x140);
	TYPE_FIELD(uint8_t body_digest[PKG_HASH_SIZE], 0x160);

	// TODO: i think these fields are actually members of element of container array
	TYPE_FIELD(uint32_t unk_0x400, 0x400);
	TYPE_FIELD(uint32_t pfs_image_count, 0x404);
	TYPE_FIELD(uint64_t pfs_flags, 0x408);
#		define PKG_PFS_FLAG_NESTED_IMAGE UINT64_C(0x8000000000000000)
	TYPE_FIELD(uint64_t pfs_image_offset, 0x410);
	TYPE_FIELD(uint64_t pfs_image_size, 0x418);
	TYPE_FIELD(uint64_t mount_image_offset, 0x420);
	TYPE_FIELD(uint64_t mount_image_size, 0x428);
	TYPE_FIELD(uint64_t package_size, 0x430);
	TYPE_FIELD(uint32_t pfs_signed_size, 0x438);
	TYPE_FIELD(uint32_t pfs_cache_size, 0x43C);
	TYPE_FIELD(uint8_t pfs_image_digest[PKG_HASH_SIZE], 0x440);
	TYPE_FIELD(uint8_t pfs_signed_digest[PKG_HASH_SIZE], 0x460);
	TYPE_FIELD(uint64_t pfs_split_size_nth_0, 0x480);
	TYPE_FIELD(uint64_t pfs_split_size_nth_1, 0x488);
TYPE_END();
CT_SIZE_ASSERT(struct pkg_header, PKG_HEADER_SIZE);

TYPE_BEGIN(struct pkg_table_entry, PKG_TABLE_ENTRY_SIZE);
	TYPE_FIELD(uint8_t raw_data[PKG_TABLE_ENTRY_SIZE], 0x00);
	TYPE_FIELD(uint32_t id, 0x00); // enum pkg_entry_id
	TYPE_FIELD(uint32_t unk1, 0x04);
	TYPE_FIELD(uint32_t flags1, 0x08);
	TYPE_FIELD(uint32_t flags2, 0x0C);
	TYPE_FIELD(uint32_t offset, 0x10);
	TYPE_FIELD(uint32_t size, 0x14);
	TYPE_FIELD(uint64_t pad, 0x18);
TYPE_END();
CT_SIZE_ASSERT(struct pkg_table_entry, PKG_TABLE_ENTRY_SIZE);
...
enum pkg_entry_id {
	...
	PKG_ENTRY_ID__PARAM_JSON = 0x00002000,
};

#define PKG_ENTRY_NAME__PARAM_SFO "param.sfo"
#define PKG_ENTRY_NAME__PARAM_JSON "param.json"
...
CT_SIZE_ASSERT(struct pkg_entry_keyset, PKG_ENTRY_KEYSET_SIZE);
...
struct pkg {
	struct file_map* map;
	struct pfs* pfs;
	struct pfs* inner_pfs;
	struct pkg_header* hdr;
	struct pkg_table_entry* entry_table;
	struct pfs_file_context* image_file;
	struct pfs_io_callbacks io;
	struct pfs_io_callbacks inner_io;
	uint64_t pfs_image_offset;
	uint64_t pfs_image_size;
	uint32_t pfs_signed_size;
	uint64_t pfs_offset;
	uint64_t inner_pfs_offset;
	size_t entry_count;
...
typedef int (*pkg_set_pfs_options_cb)(void* arg, struct pkg* pkg, struct pfs_options* opts);

typedef enum cb_result (*pkg_enum_entries_cb)(void* arg, struct pkg* pkg, struct pkg_entry_desc* desc);
...
int pkg_unpack_single(struct pkg* pkg, const char* path, const char* output_directory, pfs_unpack_pre_cb done_cb, void* done_cb_arg);
int pkg_unpack_all(struct pkg* pkg, const char* output_directory, pfs_unpack_pre_cb done_cb, void* done_cb_arg);

static inline unsigned int pkg_table_entry_key_index(struct pkg_table_entry* entry) {
	assert(entry != NULL);
	return (BE32(entry->flags2) & 0xF000) >> 12;
}

static inline int pkg_table_entry_has_new_enc_algo(struct pkg_table_entry* entry) {
	assert(entry != NULL);
	return (BE32(entry->flags1) & 0x10000000) != 0 ? 1 : 0;
}

static inline int pkg_table_entry_is_encrypted(struct pkg_table_entry* entry) {
	assert(entry != NULL);
	return (BE32(entry->flags1) & 0x80000000) != 0 ? 1 : 0;
}

static inline int pkg_is_patch(struct pkg* pkg) {
	uint32_t flags;
	int result = 0;
	assert(pkg != NULL);
	flags = BE32(pkg->hdr->content_flags);
	result |= (flags & PKG_CONTENT_FLAGS_FIRST_PATCH) != 0 ? 1 : 0;
	result |= (flags & PKG_CONTENT_FLAGS_SUBSEQUENT_PATCH) != 0 ? 1 : 0;
	result |= (flags & PKG_CONTENT_FLAGS_DELTA_PATCH) != 0 ? 1 : 0;
	result |= (flags & PKG_CONTENT_FLAGS_CUMULATIVE_PATCH) != 0 ? 1 : 0;
	return result;
}
